...
library(parallel)
library(foreach)
source("path/cellGMM.R", echo=TRUE)
set.seed(17387)
...
result.cellMCD <- foreach (i = 1:nsample, .packages = c("cellWise"), .errorhandling = "pass") %dopar% {
  cellMCD(Xout[[i]], alpha = 0.5, crit = 1e-6, noCits = 500, checkPars = list(coreOnly = TRUE, numDiscrete = 0))       
}
stopCluster(cl)
...
for (samp in 1:nsample) {
  ...
  if (!is.character(unlist(result.sclust[[samp]][1]))) {
    ARI[samp, 4] <- adjustedRandIndex(label.th, result.sclust[[samp]]$label.compl)
  ...
  if (!is.character(unlist(result.sclust[[samp]][1]))) {
    Missclass[samp, 4] <- sum(t(result.sclust[[samp]]$U) %*% Uth - diag(diag(t(result.sclust[[samp]]$U) %*% Uth)))/n.obs
  }
  if (length(result.MNM[[samp]]) > 4) {
  ...
  for (g in 1:G) {
    if (g == 1) {
      ...
      if (!is.character(unlist(result.sclust[[samp]][1]))) {
        MSE.pi[samp, g + 3] <- (pp[g] - result.sclust[[samp]]$pi[g])^2
      }
      if (length(result.MNM[[samp]]) > 4) {
      ...
      if (!is.character(unlist(result.sclust[[samp]][1]))) {
        MSE.post[samp, g + 3] <- sqrt(mean((post.th[[samp]][, g] - result.sclust[[samp]]$post[, g])^2))
      }
      if (length(result.MNM[[samp]]) > 4) {
    ...
    if (!is.character(unlist(result.sclust[[samp]][1]))) {
      MSE.mu[samp, g + 3*G] <- mean((mu[g, ] - result.sclust[[samp]]$mu[g, ])^2)
    }
    if (length(result.MNM[[samp]]) > 4) {
    ...
    if (!is.character(unlist(result.sclust[[samp]][1]))) {
      KL.sigma[samp, g + 3*G] <- tr(result.sclust[[samp]]$sigma[[g]] %*% solve(Sigma[[g]])) - p - log(det(result.sclust[[samp]]$sigma[[g]] %*% solve(Sigma[[g]])))
    }
    if (length(result.MNM[[samp]]) > 4) {
  ...
  # Ws
  Ws[samp, 1] <- sum(which(result.cellGMM[[samp]]$W==0)%in%which(Wth[[samp]]==0), na.rm = TRUE) 
  Ws[samp, 2] <- sum(which(result.cellGMM[[samp]]$W==0)%in%which(Wth[[samp]]==1), na.rm = TRUE)/(n.obs*p - n.obs*p *alpha.out)*100
  Ws[samp, 3] <- sum(which(result.cellGMM.penopt[[samp]]$W==0)%in%which(Wth[[samp]]==0), na.rm = TRUE)
  Ws[samp, 4] <- sum(which(result.cellGMM.penopt[[samp]]$W==0)%in%which(Wth[[samp]]==1), na.rm = TRUE)/(n.obs*p - n.obs*p *alpha.out)*100
  if (!is.character(unlist(result.sclust[[samp]][1]))) {
    Ws[samp, 5] <- sum(which(result.sclust[[samp]]$V==0)%in%which(Wth[[samp]]==0), na.rm = TRUE)
    Ws[samp, 6] <- sum(which(result.sclust[[samp]]$V==0)%in%which(Wth[[samp]]==1), na.rm = TRUE)/(n.obs*p - n.obs*p *alpha.out)*100
  }
  Ws[samp, 7] <- sum(which(result.tclust[[samp]]$W==0)%in%which(Wth[[samp]]==0), na.rm = TRUE)
  Ws[samp, 8] <- sum(which(result.tclust[[samp]]$W==0)%in%which(Wth[[samp]]==1), na.rm = TRUE)/(n.obs*p - n.obs*p *alpha.out)*100
  Ws[samp, 9] <- sum(which(result.MCNM[[samp]]$W==0)%in%which(Wth[[samp]]==0), na.rm = TRUE)
  Ws[samp, 10] <- sum(which(result.MCNM[[samp]]$W==0)%in%which(Wth[[samp]]==1), na.rm = TRUE)/(n.obs*p - n.obs*p *alpha.out)*100
  Ws[samp, 11] <- sum(which(result.MtM[[samp]]$W==0)%in%which(Wth[[samp]]==0), na.rm = TRUE)
  Ws[samp, 12] <- sum(which(result.MtM[[samp]]$W==0)%in%which(Wth[[samp]]==1), na.rm = TRUE)/(n.obs*p - n.obs*p *alpha.out)*100
  Ws[samp, 13] <- sum(which(result.cellMCD[[samp]]$W==0)%in%which(Wth[[samp]]==0), na.rm = TRUE)
  Ws[samp, 14] <- sum(which(result.cellMCD[[samp]]$W==0)%in%which(Wth[[samp]]==1), na.rm = TRUE)/(n.obs*p - n.obs*p *alpha.out)*100
  Ws[samp, 15] <- sum(which(result.DI[[samp]]$W==0)%in%which(Wth[[samp]]==0), na.rm = TRUE)
...
rownames(stat.W) <- c("cellGMM.pen0", "cellGMM.penb", "sclust_25", "tclust", "MCNM", "MtM", "cellMCD", "DI")
